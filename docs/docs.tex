...
\lstinline{firebase-functions} is required to create Cloud Functions and set up triggers.

\lstinline{express} is used to route requests to appropriate endpoints.

\lstinline{mysql} is used to interact with the MySQL database.

\lstinline{cors} is used to enable cross-origin resource sharing, allowing other origins to access the API endpoints, which is particularly useful when testing.
By using this as our first middleware, we enable CORS on app requests to \lstinline{api}.
...
The \lstinline{con} variable stores the MySQL connection, which is created using the host, credentials, and database name for the database in question.
...
Attempts to use the credentials stored in \lstinline{con} to connect to the database.
Creates a log message if successful, throws an error otherwise.
...
Reads the entire \lstinline{data} table from the MySQL database and sends the result to the callback function.
...
Reads all types stored in the \lstinline{types} table from the MySQL database and sends the result to the callback function.
...
Clears the existing \lstinline{data} table using \lstinline{TRUNCATE TABLE data}.

Inserts a new row for each row in the HTML table, containing all the necessary information.

Clears the existing \lstinline{types} table using \lstinline{TRUNCATE TABLE types}.
...
Handles \lstinline{POST} requests, which are used to update the database.

Passes the body of the request to the \lstinline{writeToMySQL()} function.
Sends the body back with a success code.
...
Handles \lstinline{GET} requests, which are used to retrieve data from the database.

Calls the \lstinline{readDataFromMySQL()} and \lstinline{readTypesFromMySQL()} functions, sends the return values back in an array, along with a success error code.
...
Requests which are routed by firebase to the endpoint are handled by the \lstinline{api} object.
